########################################
# Reusable IRSA trust policy builder
########################################
...
# Policy sourced from the AWS Load Balancer Controller project's published
# iam_policy.json, trimmed to the actions this platform actually performs.
...
########################################
# Runner pods IRSA — ECR push from inside the cluster
########################################
data "aws_iam_policy_document" "runner_assume" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "${local.oidc_provider_host}:sub"
      values   = ["system:serviceaccount:actions-runner-system:github-runner"]
    }
...
resource "aws_iam_policy" "runner" {
  ...
  description = "Allows runner pods to push images to ECR and read the cluster"
